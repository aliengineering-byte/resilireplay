...
export function terminalReport(metrics: RecoveryMetrics, color = true): string {
  ...
  const lines = [
    `${cyan}ResiliReplay v0.2.0${reset}  ${mark}`,
...
function reportJson(events: readonly TraceEvent[], metrics: RecoveryMetrics): string {
  return `${stableStringify({
    schemaVersion: "1.0",
    product: { name: "ResiliReplay", version: "0.2.0" },
...
<footer>Generated locally by ResiliReplay v0.2.0. No telemetry was sent.</footer>
</body></html>
`;
}
...
function badge(metrics: RecoveryMetrics): string {
  ...
  const value = metrics.passed ? "passing v0.2.0" : "failing v0.2.0";
