...
import { PRODUCT_VERSION, safeOutputPath, stableStringify } from "@resilireplay/core";
...
export function campaignTerminalReport(runInput: CampaignRun, color = true): string {
  ...
  const lines = [
    `${cyan}ResiliReplay Campaign v${PRODUCT_VERSION}${reset}  ${passing ? `${green}PASS${reset}` : `${red}FAIL${reset}`}`,
...
export function comparisonTerminalReport(
  ...
  const reset = color ? "\u001b[0m" : "";
  return [
    `${cyan}ResiliReplay Baseline v${PRODUCT_VERSION}${reset}  ${passing ? `${green}PASS${reset}` : `${red}${comparison.status.toUpperCase()}${reset}`}`,
