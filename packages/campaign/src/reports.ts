...
export function campaignTerminalReport(runInput: CampaignRun, color = true): string {
  ...
  const lines = [
    `${cyan}ResiliReplay Campaign v0.3.0${reset}  ${passing ? `${green}PASS${reset}` : `${red}FAIL${reset}`}`,
...
export function comparisonTerminalReport(
  ...
  const reset = color ? "\u001b[0m" : "";
  return [
    `${cyan}ResiliReplay Baseline v0.3.0${reset}  ${passing ? `${green}PASS${reset}` : `${red}${comparison.status.toUpperCase()}${reset}`}`,
