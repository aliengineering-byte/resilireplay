...
export async function runAdopt(options: AdoptOptions = {}): Promise<AdoptResult> {
  ...
  if (options.dryRun) {
    return {
      ...
      createdFiles: [],
      nextCommands: [
        `npx --yes resilireplay@0.7.0 adopt --config ./${plan.config} --server ${server}`,
      ],
    };
  }
  ...
  try {
    ...
    const localCommand = `npx --yes resilireplay@0.7.0 campaign run ${outputDirectory}/campaign.yml --confirm-tools ${reviewedCampaignHash}${remoteArgument}`;
