...
for (const manifest of [codex, claude]) {
  ...
  invariant(manifest.version === "0.7.0", "Plugin version must match the release");
...
invariant(
  mcp.mcpServers?.resilireplay?.args?.includes("resilireplay@0.7.0"),
...
for (const [path, schemaVersion] of publicSchemas) {
  ...
  invariant(
    schema.$id === `https://github.com/aliengineering-byte/resilireplay/blob/v0.7.0/${path}`,
