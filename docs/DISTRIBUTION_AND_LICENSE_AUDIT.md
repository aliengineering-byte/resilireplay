...
consistent with `Apache-2.0`. The npm account contact decision noted below does not change package
authorship or license metadata.
...
| npm registry                       | Public `resilireplay@0.3.0` and `resilireplay@0.3.1` report `Apache-2.0`; the synchronized `0.3.1` manifest retains the approved public author identity.                                                                  |
...
- npm derives the `maintainers` contact from the npm account profile. The user completed npm's
  security verification and intentionally retained the working account email because the GitHub
  noreply address cannot receive npm verification, recovery, or security messages. That account-level
  contact is not copied into new repository content; package author metadata remains the approved
  public identity. No credential or private address is recorded here.
...
- `npm view resilireplay@0.3.1 ...` (via the configured package runtime) — public registry license,
  author, site, repository, tarball, and integrity metadata; a clean `pnpm dlx` returned `0.3.1`.
