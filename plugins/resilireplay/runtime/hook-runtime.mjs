...
var EventTypeSchema = external_exports.enum(EVENT_TYPES);
var FaultMetadataSchema = external_exports.object({
  scenarioId: external_exports.string().min(1),
  faultType: external_exports.string().min(1),
  seed: external_exports.number().int(),
  applicationIndex: external_exports.number().int().nonnegative(),
  originalPayloadHash: external_exports.string().regex(/^[a-f0-9]{64}$/),
...
var TraceEventSchema = external_exports.object({
  ...
  parentId: external_exports.string().min(1).optional(),
  causeId: external_exports.string().min(1).optional(),
  ...
  payloadHash: external_exports.string().regex(/^[a-f0-9]{64}$/),
...
var FaultTypeSchema = external_exports.enum(FAULT_TYPES);
var FaultRuleSchema = external_exports.object({
  fault: FaultTypeSchema,
  event: external_exports.enum(EVENT_TYPES).optional(),
  occurrence: external_exports.number().int().positive().default(1),
  probability: external_exports.number().min(0).max(1).default(1),
  parameters: external_exports.record(external_exports.unknown()).default({})
}).strict();
var FaultScenarioSchema = external_exports.object({
  schemaVersion: external_exports.literal("1.0").default("1.0"),
  id: external_exports.string().min(1),
  description: external_exports.string().min(1),
  seed: external_exports.number().int().default(42),
  rules: external_exports.array(FaultRuleSchema).min(1)
}).strict();
var BUILTIN_SCENARIOS = {
  ...
  "tool-timeout": FaultScenarioSchema.parse({
    ...
    seed: 42,
    rules: [{ fault: "timeout", event: "tool_result", parameters: { timeoutMs: 1e3 } }]
  }),
...
var ADAPTER_MANIFEST_SCHEMA = "resilireplay.adapter-manifest/v1";
var Sha256 = external_exports.string().regex(/^[a-f0-9]{64}$/u);
var BoundedText = external_exports.string().max(512);
var AgentSourceSchema = external_exports.enum(["claude-code", "codex", "hermes", "generic"]);
var AgentEventSchema = external_exports.object({
  schemaVersion: external_exports.literal(AGENT_EVENT_SCHEMA),
  eventId: Sha256,
  source: AgentSourceSchema,
  sessionId: Sha256,
  eventType: external_exports.enum(["tool-result", "session-end", "turn-end"]),
  toolName: BoundedText.optional(),
  toolCallId: Sha256.optional(),
  parentId: Sha256.optional(),
  outcome: external_exports.enum(["succeeded", "failed", "interrupted", "unknown"]),
  durationMs: external_exports.number().int().nonnegative().max(864e5).optional(),
  errorClass: external_exports.enum([
    "none",
    "process-exit",
    "timeout",
    "permission",
...
var CaptureSessionSchema = external_exports.object({
  schemaVersion: external_exports.literal(CAPTURE_SESSION_SCHEMA),
  sessionId: Sha256,
  status: external_exports.enum(["armed", "stopped"]),
  startedAt: external_exports.string().datetime(),
  stoppedAt: external_exports.string().datetime().optional(),
  eventCount: external_exports.number().int().nonnegative().max(2e4),
  failureCount: external_exports.number().int().nonnegative().max(2e4),
  limits: external_exports.object({ maxEvents: external_exports.literal(2e4), maxEventBytes: external_exports.literal(32768) }).strict()
}).strict();
var FailureEvidenceSchema = external_exports.object({
  schemaVersion: external_exports.literal(FAILURE_EVIDENCE_SCHEMA),
  evidenceId: Sha256,
  source: AgentSourceSchema,
  sessionId: Sha256,
...
var AdapterManifestSchema2 = external_exports.object({
  ...
  license: external_exports.literal("Apache-2.0"),
  source: AgentSourceSchema,
  entrypoint: external_exports.string().max(240),
  events: external_exports.array(external_exports.enum(["tool-result", "session-end", "turn-end"])).min(1),
  privacy: external_exports.object({
    rawPromptsPersisted: external_exports.literal(false),
    rawTranscriptsPersisted: external_exports.literal(false),
    environmentValuesPersisted: external_exports.literal(false)
  }).strict()
}).strict();
...
var MAX_HOOK_BYTES = 1048576;
var MAX_EVENT_BYTES = 32768;
var MAX_EVENTS = 2e4;
function hashValue2(value) {
  return createHash("sha256").update(stableStringify(value)).digest("hex");
}
function boundedSummary(value, limit = 512) {
  if (value === void 0 || value === null) return void 0;
  const text2 = (typeof value === "string" ? value : stableStringify(value)).replace(SECRET, "[REDACTED]").replace(PATH, "[PATH]").replace(CONTROL, " ").replace(/\s+/gu, " ").trim();
...
function number(value) {
  return typeof value === "number" && Number.isFinite(value) && value >= 0 ? Math.min(Math.trunc(value), 864e5) : void 0;
}
function timestamp(value) {
  if (typeof value !== "string" || Number.isNaN(Date.parse(value))) return void 0;
  return new Date(value).toISOString();
}
function classify(message, outcome) {
...
function codexOutcome(response) {
...
function normalizeHookEvent(input, options) {
  ...
  const isEnd = lowerHook.includes("stop") || lowerHook.includes("sessionend") || lowerHook.includes("session_end");
  const eventType = isEnd ? lowerHook.includes("session") ? "session-end" : "turn-end" : "tool-result";
  ...
  const outcome = isEnd ? "unknown" : failureHook ? root.is_interrupt === true ? "interrupted" : "failed" : options.source === "claude-code" && lowerHook === "posttooluse" ? "succeeded" : codexOutcome(response);
  ...
  const summary = boundedSummary(error ?? (outcome === "failed" ? response : void 0));
  const base = {
    schemaVersion: AGENT_EVENT_SCHEMA,
    source: options.source,
    sessionId: hashValue2([options.source, sessionRaw]),
    eventType,
    outcome,
    capturedAt: options.receivedAt ?? timestamp(root.timestamp ?? root.captured_at ?? root.capturedAt) ?? (/* @__PURE__ */ new Date()).toISOString(),
    ...text(root.tool_name ?? root.toolName) ? { toolName: boundedSummary(root.tool_name ?? root.toolName) } : {},
    ...toolCallRaw ? { toolCallId: hashValue2([options.source, sessionRaw, toolCallRaw]) } : {},
    ...parentRaw ? { parentId: hashValue2([options.source, sessionRaw, parentRaw]) } : {},
    ...number(root.duration_ms ?? root.durationMs) !== void 0 ? { durationMs: number(root.duration_ms ?? root.durationMs) } : {},
    errorClass: classify(summary, outcome),
    ...toolInput !== void 0 ? { inputSha256: hashValue2(toolInput) } : {},
    ...Object.keys(response).length > 0 ? { outputSha256: hashValue2(response) } : {},
    ...summary ? { summary } : {}
  };
...
async function withLock(rootInput, operation) {
...
async function repairPartialJournal(path) {
  let handle;
  try {
  ...
  await atomicWrite(
    path,
    recovered.length === 0 ? "" : `${recovered.map((entry) => stableStringify(entry)).join("\n")}
`
  );
}
...
async function captureIngestBatch(input, root = process.cwd()) {
  ...
  return withLock(root, async () => {
    ...
    try {
      await Promise.all(
        [...shards.values()].filter((shard) => shard.added.length > 0).map((shard) => appendDurably(shard.path, shard.added.join("\n")))
      );
      ...
      await Promise.all(
        [...shards.values()].filter((shard) => shard.added.length > 0).map((shard) => atomicWrite(shard.path, shard.original))
      );
      throw error;
    }
...
`);
    let recent = [];
    try {
      const value = JSON.parse(await readFile(location.recent, "utf8"));
      if (Array.isArray(value)) {
        recent = value.filter(
          (entry) => typeof entry === "object" && entry !== null && typeof entry.sessionId === "string" && typeof entry.eventId === "string"
        ).slice(-16);
      }
    } catch {
      recent = [];
    ...
    for (const { event } of accepted) {
      const causalEventIds = recent.filter((entry) => entry.sessionId === event.sessionId).map((entry) => entry.eventId);
      ...
      if (event.outcome === "failed" || event.outcome === "interrupted") {
        ...
        lastEvidence = FailureEvidenceSchema.parse({ ...body, evidenceId: hashValue2(body) });
      }
    }
...
async function captureIngest(event, root = process.cwd()) {
